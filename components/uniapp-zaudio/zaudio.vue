<template>
	<view class="imt-audio" :class="[`${theme}`]" v-if="audiolist.length > 0">
        ...
        <view class="top_img">
			<view class="img">
				<image :src="require('./static/erji.png')" mode=""></image>
			</view>
			<view class="audio-button-box"style="flex-grow: 1;">
				<!-- 块退15s -->
				<!-- <image :src="require('./static/prev.png')" class="prevbtn" @click="stepPlay(-15)" mode="widthFix"></image> -->
				<!-- 上一首 -->
				<view class="left">
					<image :src="require('./static/xiayige.png')" class="prevplay" @click="changeplay(-1)" mode="widthFix"></image>
					<text>上一首</text>
				</view>
				
				
				<view class="playbox">
					<image :src="require('./static/bofang.png')" v-if="loading" class="play loading"></image>
					
					<view v-else>
						<!-- 播放 -->
						<view @click="operate" class="play" v-if="renderData('paused')">
							<image :src="require('./static/bofang.png')"  ></image>
							<text>播放</text>
						</view>
						<!-- 暂停 -->
						
						<view @click="operate" class="pause" v-else>
							<image :src="require('./static/zanting.png')"  ></image>
							<text>暂停</text>
				...
				<!-- 下一首 -->
				<view class="right">
					<image :src="require('./static/xiayige.png')" class="nextplay" @click="changeplay(1)" mode="widthFix"></image>
					<text>下一个</text>
				</view>
		...
		<view class="bot">
			<view class="bot_left" style="font-size: 32rpx;color: #4d4d4d;font-weight: 400;height: 51rpx;line-height: 51rpx;background-color: #DFDFDF;border-radius: 29rpx;margin: 15rpx;">年内多地调整公积金</view>
			<view class="audio-wrapper">
